...
final class QuoteViewController: UIViewController {
    ...
    @IBOutlet private weak var nextLabel: UILabel! {
        didSet { nextLabel.text = R.string.localizable.next() }
    }
    @IBOutlet private weak var categoryButton: AnimateButton! {
        didSet { categoryButton.layer.smoothCorner(8) }
    }
    @IBOutlet private weak var templateButton: AnimateButton! {
        didSet { templateButton.layer.smoothCorner(8) }
    }
        ...
        viewModel.selectedTemplate
            ...
            .drive(onNext: { [weak self] selectedTemplate in
                ...
                if let selectedTemplate = selectedTemplate,
                   let selectedTemplateImage = UIImage(named: "\(selectedTemplate).jpg", in: Bundle.main, with: nil) {
                    self.backgroundImageView.image = selectedTemplateImage
                    self.opaqueView.alpha = 0.4
                } else {
    ...
    @IBAction private func templateButtonDidTap(_ sender: AnimateButton) {
        viewModel.presentTemplateViewController()
    }
}
